...
abstract public class AbstractDemo implements Demo, ProductNames {
    ...
    public static JFrame showAsFrame(final Demo demo) {
        final JFrame frame = new JFrame(MessageFormat.format(Resource.RB.getString("Demo.title"), demo.getName(), Lm.getProductVersion(), SystemInfo.getJavaVersion()));
...
//        com.jidesoft.swing.JideSwingUtilities.traceFocus();
    ...
    protected static JComponent createOptionsPanel(JFrame parentFrame, Demo demo, Component demoPanel) {
        ...
        if (demoPanel != null) {
            ...
            if (optionsPanel != null) {
                CollapsiblePane pane = createCollapsiblePane(Resource.RB.getString("Demo.options"));
            ...
            if (demo.isCommonOptionsPaneVisible()) {
                CollapsiblePane pane = createCollapsiblePane(Resource.RB.getString("Demo.commonOptions"));
            ...
            if (description != null && description.trim().length() > 0) {
                CollapsiblePane pane = createCollapsiblePane(Resource.RB.getString("Demo.description"));
            ...
            if (source != null && source.length > 0) {
                CollapsiblePane pane = createCollapsiblePane(Resource.RB.getString("Demo.source"));
                JPanel panel = new JPanel(new BorderLayout(4, 4));
                StringBuffer sourceFiles = new StringBuffer(MessageFormat.format(Resource.RB.getString("Demo.location"), "examples/" + demo.getDemoFolder()));
                for (String s : source) {
                    sourceFiles.append("\n  - ");
